...
def logistic_train(Xtrain, ytrain, w, b, step_size, max_iterations):
    ...
    w = numpy.insert(w,0,0,axis=0)



    N = Xtrain.shape[0]
    for iter in range(max_iterations):
        h = numpy.matmul(Xtrain, w)
        sigma = numpy.divide(numpy.exp(h), numpy.add(1, numpy.exp(h)))
...
def logistic_test(Xtest, ytest, w, b, t = 0.5):
    ...
    sigma = numpy.divide(1,numpy.add(1,numpy.exp(-1 * numpy.matmul(Xtest,w))))
    ypred = numpy.zeros(shape=ytest.shape)
