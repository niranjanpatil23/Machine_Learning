...
def regularized_linear_regression(X, y, lambd):
    ...
    Xt = np.transpose(X)
    eye = np.eye(Xt.shape[0]);

    w = np.matmul(np.matmul(np.linalg.inv(np.matmul(Xt, X) + lambd*eye), Xt), y)
...
def tune_lambda(Xtrain, ytrain, Xval, yval, lambds):
    ...
    for i in range(len(lambds)):
        ...
        w = np.matmul(np.matmul(np.linalg.inv(np.matmul(Xt, Xtrain) + lambds[i] * eye), Xt), ytrain)
        mse = np.sum(np.subtract(np.matmul(Xval,w),yval) ** 2) / Xval.shape[0]
...
def test_error(w, X, y):
    ...
    err = np.sum(np.subtract(np.matmul(X, w), y) ** 2) / X.shape[0]
