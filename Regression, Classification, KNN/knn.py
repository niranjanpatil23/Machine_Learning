...
def compute_distances(Xtrain, X):
    ...
    dists = np.sqrt((X ** 2).sum(axis=1)[:, np.newaxis] + (Xtrain ** 2).sum(axis=1) - 2 * X.dot(Xtrain.T))  # studied in lecture 10. Taught by TA on 21st sept 2017
...
def predict_labels(k, ytrain, dists):
    ...
    ypred = np.zeros(shape=ytrain.shape)
    for i in range(len(dists)):
        dict = {}
        for j in range(k):
            label = ytrain[dists[i][j]]
...
def find_best_k(K, ytrain, dists, yval):
    ...
    minAcc = -1
    validation_accuracy = np.zeros(shape=len(K))
    best_k = -1
    cnt = 0
    for k in K:
        ypred = predict_labels(k, ytrain, dists)
        acc = compute_accuracy(yval,ypred)
        validation_accuracy[cnt] = acc
        cnt += 1
        if acc > minAcc:
            minAcc = acc
            best_k = k
