...
def nb_train_smoothing(X, Y, vocab_size, num_classes, alpha):
    ...
    ###################################################
    # Q8.2 Edit here
    for x in range(len(class_word_prob[0])):
        class_word_prob[0][x] = alpha
        class_word_prob[1][x] = alpha
    wc = [0, 0]
    for i in range(len(Y)):
        ...
        for j in range(len(X[i])):
            word_id = X[i][j][0]
            word_f = X[i][j][1]
            class_word_prob[Y[i]][word_id] += word_f
    wc[0] = sum(class_word_prob[0])
    wc[1] = sum(class_word_prob[1])
    # print('hey')
    class_word_prob[0] /= wc[0]
    class_word_prob[1] /= wc[1]
    class_prob /= len(Y)
    # print('hey')
    ###################################################
...
def nb_predict(X, class_prob, class_word_prob):
    ...
    for i in range(len(X)):
        prob = [np.log(class_prob[0]),np.log(class_prob[1])]
        for j in range(len(X[i])):
            word_id = X[i][j][0]
            word_f = X[i][j][1]
            if class_word_prob[0][word_id] != 0 and class_word_prob[1][word_id] != 0:
                prob[0] += word_f * np.log(class_word_prob[0][word_id])
                prob[1] += word_f * np.log(class_word_prob[1][word_id])
            elif class_word_prob[0][word_id] != 0 and class_word_prob[1][word_id] == 0:
                prob[0] += word_f * np.log(class_word_prob[0][word_id])
                prob[1] += word_f * (-1e50)
            elif class_word_prob[1][word_id] != 0 and class_word_prob[0][word_id] == 0:
                prob[0] += word_f * (-1e50)
                prob[1] += word_f * np.log(class_word_prob[1][word_id])
