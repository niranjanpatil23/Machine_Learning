...
def main(main_params):
    ...
    for t in range(num_epoch):
        ...
        for i in range(int(np.floor(N_train / minibatch_size))):
            ...
            ax = model['C2'].forward(m1)
            hx = model['nonlinear2'].forward(ax)
            m2 = model['M2'].forward(hx)
            ...
            # TODO (3)
            grad_hx = model['M2'].backward(hx, grad_m2)
            grad_ax = model['nonlinear2'].backward(ax, grad_hx)
            grad_m1 = model['C2'].backward(m1, grad_ax)
