...
def logistic_train_ovr(Xtrain, ytrain, w, b, step_size, max_iterations):
    ...
    for iter in range(max_iterations):
        h = numpy.matmul(Xtrain, w) + b
        sigma = sigmoid(h);
        sig_y = numpy.subtract(sigma, ytrain)
        theta = numpy.zeros(shape=w.shape)
        b -= step_size * numpy.sum(sig_y) / N
        #for i in range(w.shape[0]):
        #    theta[i] = w[i] - numpy.multiply(step_size / N, numpy.sum(numpy.multiply(sig_y, Xtrain[:, i])))
        grad = numpy.dot(numpy.transpose(Xtrain),sig_y)
...
def logistic_test_ovr(Xtest, w_l, b_l):
    ...
    """
    # you need to fill in your solution here
    test_pred = sigmoid(numpy.dot(Xtest,w_l) + b_l)
    return test_pred
...
def logistic_mul_train(Xtrain, ytrain, w, b, step_size, max_iterations):
    ...
    # you need to fill in your solution here
    '''
            for cl in range(ytrain.shape[1]):
                b[cl] -= step_size * numpy.sum(soft_y[cl]) / N
                theta = numpy.zeros(shape=w_t.shape[0])
                for dim in range(w_t.shape[0]):
                    X_n = Xtrain[:, dim]
                    sf = soft_y[:,cl]
                    mal = numpy.multiply(sf, X_n)
                    summ = numpy.sum(mal)
                    theta[dim] = w_t[dim][cl] - numpy.multiply(step_size / N, summ)
                grad[:,cl] = theta
            '''
    #print('hey')
    ...
    N = Xtrain.shape[0]
    for iter in range(max_iterations):
        h = numpy.dot(Xtrain, w_t) + b[None,:]
        d = h - numpy.max(h, axis=1)[:, None]
        d = numpy.exp(d)
        sum_d = numpy.sum(d, axis=1)
        softmax = d / sum_d[:,None]
        soft_y = softmax - ytrain
        b -= numpy.sum(soft_y,axis=0) * step_size/N
        grad = numpy.dot(numpy.transpose(Xtrain),soft_y)
        w_t -= grad * step_size/N
        #print('hey')
    w_l = numpy.transpose(w_t)
    b_l = b
...
def logistic_mul_test(Xtest, w_l, b_l):
    ...
    """
    # you need to fill in your solution here
    w_t = numpy.transpose(w_l)
    h = numpy.dot(Xtest, w_t) + b_l[None, :]
    d = h - numpy.max(h, axis=1)[:, None]
    d = numpy.exp(d)
    sum_d = numpy.sum(d, axis=1)
    test_pred = d / sum_d[:, None]
    #print('hey')
    return test_pred
