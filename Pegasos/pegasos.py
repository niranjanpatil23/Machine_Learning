...
def objective_function(X, y, w, lamb):
    ...
    ywx = 1 - np.multiply(y, np.dot(X,w)[:,0])
    obj_value = lamb*0.5*np.sum(w**2) + np.sum((ywx>0).astype(int))/X.shape[0]
    return obj_value
...
def pegasos_train(Xtrain, ytrain, w, lamb, k, max_iterations):
    ...
    for iter in range(1, max_iterations + 1):
        ...
        At_p = A_t[np.where(np.multiply(ytrain[A_t], np.dot(Xtrain[A_t],w)[:,0]) < 1)]
        n_t = 1 / (lamb * iter)
        pq = np.transpose(np.sum(np.multiply(ytrain[At_p][:,None],Xtrain[At_p]),axis=0))
        w_t = (1 - n_t*lamb)* w + (n_t/k)* pq.reshape(w.shape)
        #w = w_t * min(1, 1/((lamb*np.sum(w_t))**(0.5)))
        w = w_t * min(1, 1 / ((lamb * np.sum(w_t**2)) ** (0.5)))
